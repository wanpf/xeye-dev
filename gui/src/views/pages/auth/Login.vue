		...
		.then(res => {
			if(res?.data?.token){
				...
				setTimeout(()=>{
				  ...
				  router.push(isAdmin() ?'/server':'/client');
				},1000);
			} else {
...
<template>
    <div style="overflow: hidden;" class="surface-ground flex align-items-center justify-content-center  min-w-screen overflow-hidden">
        <Toast position="bottom-center" group="login" @close="onClose">
				...
				<svg style="opacity: 0.2;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 800" class="fixed left-0 top-0 min-h-screen min-w-screen" preserveAspectRatio="none"><rect fill="var(--primary-500)" width="1600" height="800"></rect><path fill="var(--primary-400)" d="M478.4 581c3.2 0.8 6.4 1.7 9.5 2.5c196.2 52.5 388.7 133.5 593.5 176.6c174.2 36.6 349.5 29.2 518.6-10.2V0H0v574.9c52.3-17.6 106.5-27.7 161.1-30.9C268.4 537.4 375.7 554.2
				...
				<div class=" flex flex-column align-items-center justify-content-center" style="position: relative;margin-top: 5%;">
						<Avatar icon="pi pi-lock"  height="50" class="mb-5" size="xlarge" shape="circle"/>
            <div style="border-radius: 20px; padding: 0.2rem;">
                <div class="shadow w-full surface-card py-7 px-4 sm:px-8" style="border-radius: 20px;">
                    <div class=" mb-5">
